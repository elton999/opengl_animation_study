...
bool Shader::LinkShader(unsigned int vertex, unsigned int fragment)
{
	...
	if (!success)
	{
		...
		std::cout << "ERROR: Sheder linking failed.\n";
		std::cout << "\t" << infoLog << "\n";
...
void Shader::PopulateUniforms()
{
	...
	{
		memset(name, 0, sizeof(char) * 128);
		glGetActiveUniform(mHandle, (GLuint)i, 128, &lenght, &size, &type, name);
		...
		if (uniform >= 0)
		{
			...
			if (found != std::string::npos)
			{
				...
				while (true)
				{
					...
					int uniformLocalization = glGetUniformLocation(mHandle, testName);
					if (uniformLocalization < 0)
					{
						break;
					}

					uniform[testName] = uniformLocalization;
				}
			}
